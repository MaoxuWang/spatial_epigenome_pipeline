...
ap.add_argument("-i2", "--block_size", type=int, default = 35, 
    help="block size for thresholding")
ap.add_argument("-i3", "--C_normalized", type=float, default = 0.1, 
    help="normalized value to determine the in tissue pixel (0-1)")
ap.add_argument("-i4", "--img_res", type=int, 
    default = 1300,
    help="image resolution (the smaller one)")
ap.add_argument('-o', '--outdir', type=str, default="output directory")
ap.add_argument('--decode_file', required = True)
...
if not os.path.isfile(os.path.join(outdir, 'ROI_binary.png')):
    binary_adaptive_inverted_img.save(
        os.path.join(outdir, 'ROI_binary.png')
    )
else:
...
# Read image
I = cv2.imread(os.path.join(outdir, 'ROI_binary.png'), 0)
...
df_in_tissue[4] = (4*(df_in_tissue[1])+1)*pixel_ratio
