...
def bam2fragments(
    ...
    """
    ### out file 
    sampleid = sampleid + "_" + barcode_tag
    out_fragment = os.path.join(output_dir, sampleid + ".fragments.tsv.gz")
    out_tss = os.path.join(output_dir, sampleid + ".tss_enrichment.pdf")
    out_gene_matrix_h5 = os.path.join(output_dir, sampleid + ".gene_activity.h5ad")

    # default: 5kb
    if _bin_size == "5000":
        out_file_h5 = os.path.join(output_dir, sampleid +".h5ad")
        out_metadata = os.path.join(output_dir, sampleid + ".metadata.txt")
        outpdf = os.path.join(output_dir, sampleid + ".cluster.pdf")
    else:
        out_file_h5 = os.path.join(output_dir, sampleid + "." + _bin_size +".h5ad")
        out_metadata = os.path.join(output_dir, sampleid + "." + _bin_size + ".metadata.txt")
        outpdf = os.path.join(output_dir, sampleid + "." + _bin_size + ".cluster.pdf")
    ...
    if tissue_barcode[-2::] == "gz":
        cells = pd.read_csv(tissue_barcode, header=None, compression='gzip')
    else:
